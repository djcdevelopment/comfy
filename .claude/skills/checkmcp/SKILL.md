...
`C:\work\commandcenter`, not in this repo (`C:\work\comfy`). The `cd` above is
required so `python -m hearth.callers.doorcheck` can find the module — run it
that way regardless of your session's working directory.

It checks four layers and prints a verdict; exit 0 = HEALTHY, exit 1 = DEGRADED:
...
- **mcp** — real MCP handshake + tool count (should be ~35 tools). "degraded"
