...
Canonical node map: `C:\work\commandcenter\fleet\inventory.toml`.

**Cross-repo note:** the venv and the `hearth`/`fleet` Python packages live in
`C:\work\commandcenter`, not in this repo (`C:\work\comfy`). The `cd` below is
required so `python -m fleet.fleet_ping` / `hearth.callers.doorcheck` can find
the modules — run it that way regardless of your session's working directory.

Run both:

```
cd /c/work/commandcenter && ./fleet-worker-node/.venv-omen/Scripts/python.exe -m fleet.fleet_ping --all-services --no-color
cd /c/work/commandcenter && ./fleet-worker-node/.venv-omen/Scripts/python.exe -m hearth.callers.doorcheck --revive
```
