"""Local Ollama tools for the Comfy MCP gateway."""
...
def local_generate(
    ...
    try:
        ...
        return {"ok": False, "error": f"{type(exc).__name__}: {exc}", "endpoint": resolved_endpoint, "model": model}
    except json.JSONDecodeError as exc:
        return {"ok": False, "error": f"non-JSON response from Ollama: {exc}", "endpoint": resolved_endpoint, "model": model}
